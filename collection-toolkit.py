...
def get_interfaces_ip(task):
    task.run(name="Get interfaces IP",
             task=napalm_get,
...
def get_lldp_neighbors(task):
    task.run(name="Get LLDP neighbors",
             task=napalm_get,
...
def main_collector(wb):
    ...
    # Statically assign headers
    lldp_nei_headers = ['Local Hostname', 'Local Port', 'Remote Hostname', 'Remote Port']
    # Write headers on the top line of the file
    ...
    junos_interfaces_ip = junos_devices.run(name="Processing interface IP addresses",
                                        task=get_interfaces_ip)
    eos_interfaces_ip = eos_devices.run(name="Processing interface IP addresses",
                                        task=get_interfaces_ip)
    nxos_interfaces_ip = nxos_devices.run(name="Processing interface IP addresses",
                                        task=get_interfaces_ip)
    iosxr_interfaces_ip = iosxr_devices.run(name="Processing interface IP addresses",
                                        task=get_interfaces_ip)
    # IOS Platform Block
    ...
    can be parsed and saved to a spreadsheet
    '''
    ios_lldp = ios_devices.run(name="Processing LLDP neighbors",
                    task=get_lldp_neighbors)
    junos_lldp = junos_devices.run(name="Processing LLDP neighbors",
                    task=get_lldp_neighbors)
    eos_lldp = eos_devices.run(name="Processing LLDP neighbors",
                    task=get_lldp_neighbors)
    nxos_lldp = nxos_devices.run(name="Processing LLDP neighbors",
                    task=get_lldp_neighbors)
    iosxr_lldp = iosxr_devices.run(name="Processing LLDP neighbors",
                    task=get_lldp_neighbors)
    # IOS Platform Block
